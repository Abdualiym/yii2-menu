...
/* @var $model domain\modules\menu\forms\menu\MenuForm */
/* @var $menu domain\modules\menu\entities\Menu */
...
<div class="menu-form">
    ...
    <div class="box box-default">
        ...
        <div class="box-body">
            ...
            <?= $form->field($model, 'type')->label('Тип меню')->dropDownList($menuTypes,['prompt'=>'Выберите...']) ?>
...
$script = <<< JS
    ...
    for(var i=0; i < categoryKeys.length; i++)
        {
            optionsCat += '<option value="'+categoryKeys[i]+'">'+categoriesList[categoryKeys[i]]+'</option>';
        }
    for(var i=0; i < textKeys.length; i++)
        {
            optionsText += '<option value="'+textKeys[i]+'">'+textsList[textKeys[i]]+'</option>';
        }
    for(var i=0; i < actionsKeys.length; i++)
        {
            optionsAction += '<option value="'+actionsKeys[i]+'">'+actionsList[actionsKeys[i]]+'</option>';
        }
    if(!currentType){return ''};
            switch (currentType){
                case 'link':
                    return '<label class="control-label" for="menuform-type_helper">Введите ссылку</label>'
                            +'<input type="text" id="menuform-type_helper" class="form-control" name="MenuForm[type_helper]" maxlength="255" placeholder="http://">';
    ...
    type.on('change',function() {
        menu_types.html('<div class="form-group field-menuform-type_helper">'
                        +getHtml($(this).val(),categoriesList,textsList,actionsList)
                        +'<div class="help-block"></div></div>'
                        
                        
                        );
        menu_types.hide(0);
        menu_types.slideDown(200);
    }); 
